...
public class CS400Graph<T> implements GraphADT<T> {
    ...
    protected Path dijkstrasShortestPath(T start, T end) {
        // if either start or end is not in the graph, throws an exception
        if(!vertices.containsKey(start) || !vertices.containsKey(end)) {
            throw new NoSuchElementException();
        }

        // sets up the priority queue of the shortest path
        PriorityQueue<Path> shortestPath = new PriorityQueue<Path>();
        Path startNode = new Path(vertices.get(start)); // creates a new path with just the start node
        shortestPath.add(startNode); // the first path added is just the startNode path

        // initializes the frontier of vertices
        ArrayList<Vertex> frontier = new ArrayList<Vertex>();

        // if the queue has no more paths, then there is no path from start to end
        while(shortestPath.peek() != null) {
            Path main = shortestPath.peek(); // the shortest path in the queue
            Vertex endVertex = main.end; // the ending vertex

            // if the frontier does not already contain the end node, it visits that node
            if(!frontier.contains(endVertex)) {
                // adds the end vertex to the frontier
                frontier.add(endVertex);

                if(frontier.contains(vertices.get(end))) {
                    return shortestPath.peek();
                    // if the frontier contains the end vertex, returns the shortest path in the queue
                }

                // Loops through each outgoing edge of the ending vertex
                for(Edge edge: endVertex.edgesLeaving) {
                    // if the frontier does not already contain the target of the edge,
                    // it creates a new path with the edge
                    if(!frontier.contains(edge.target)) {
                        shortestPath.add(new Path(main, edge));
                    }
                }
            }
            shortestPath.remove(main); // removes the main path as all other paths have been found
        }

        throw new NoSuchElementException(); // if no path can be found from start to end
    }
